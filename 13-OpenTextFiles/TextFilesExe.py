...
def write_lines_to_file(filename: str, lines: list) -> None:
    ...
    with open(filename, "w") as f:
        for i, line in enumerate(lines):
            if i == len(lines) - 1:  # Kui see on viimane rida
                f.write(line)  # Ei lisa \n, kui viimane element lõppeb sellega
            else:
                f.write(line + "\n")
...
def merge_dates_and_towns_into_csv(dates_filename: str, towns_filename: str, csv_output_filename: str) -> None:
    ...
    """
    dates_dict = {}
    with open(dates_filename, "r", encoding="utf-8") as f_dates:
        reader = csv.reader(f_dates, delimiter=":")
        for row in reader:
            name, date = row
            dates_dict[name] = date

    # Loeme linnade faili ja salvestame need sõnastikku
    towns_dict = {}
    with open(towns_filename, "r", encoding="utf-8") as f_towns:
        reader = csv.reader(f_towns, delimiter=":")
        for row in reader:
            name, town = row
            towns_dict[name] = town

    # Valmistame andmed CSV faili kirjutamiseks
    with open(csv_output_filename, "w", newline='', encoding="utf-8") as f_out:
        writer = csv.writer(f_out)
        writer.writerow(["name", "town", "date"])  # Kirjutame päise

        # Loeme kuupäevade faili uuesti, et säilitada järjekord
        with open(dates_filename, "r", encoding="utf-8") as f_dates:
            reader = csv.reader(f_dates, delimiter=":")
            for row in reader:
                name, _ = row  # Kuupäev on meil juba salvestatud sõnastikku
                town = towns_dict.get(name,
                                      "-")  # Kui linn pole olemas, siis määrame "-"
                date = dates_dict.get(name,
                                      "-")  # Kui kuupäev pole olemas, siis määrame "-"
                writer.writerow([name, town, date])

        # Käime linnade faili läbi, et lisada nimed, mis ei leidnud vastet kuupäevade failis
        with open(towns_filename, "r", encoding="utf-8") as f_towns:
            reader = csv.reader(f_towns, delimiter=":")
            for row in reader:
                name, _ = row  # Linn on meil juba salvestatud sõnastikku
